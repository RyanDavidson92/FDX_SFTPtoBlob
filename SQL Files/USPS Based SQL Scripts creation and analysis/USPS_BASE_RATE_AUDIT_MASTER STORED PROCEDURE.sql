...
-------- This script is the first attempt to get the usps stored procedure into an actual working procedure on azure. 
...
exec [TestDB].[dbo].[USPS_base_rate_audit_master] 274889
...
BEGIN
    ...
    DELETE FROM TestDB.dbo.usps_base_rate_audit 
   ...
   WITH RankedMatches AS (
    ...
    INSERT INTO TestDB.dbo.usps_base_rate_audit (
        childid, [Tracking number], controlno, Shipdate,
        [Service], BilledWeight_LB, length, width, height, [Billed_Zone], [cubic feet],[Package Indicator],
...
UPDATE TestDB.dbo.usps_base_rate_audit
SET IsRated = 1
WHERE controlno = @ywControlno
