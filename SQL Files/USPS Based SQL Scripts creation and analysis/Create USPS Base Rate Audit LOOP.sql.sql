...
CREATE PROCEDURE dbo.USPS_base_rate_audit_loop
AS
BEGIN
    SET NOCOUNT ON;
 ...
 /* add controlno's that need to be audited. This looks at USPS childids with a NULL value for ContractRate which means they
   have not been audited yet. It only looks at controlnos from the last 8 weeks. */    
   ...
   INSERT INTO #tempControlNoUSPSAudit (ControlNo)
    ...
    WHERE cm.controlDate >= DATEADD(WEEK, -8, GETDATE())
      AND cm.Carrier = 'USPS'
