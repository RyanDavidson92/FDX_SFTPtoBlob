# v6_insert_controlno_to_controlmaster_fdx.py
# FDX-only version of V6 with duplicate skipping, controlno tracking, and Azure blob upload
...
# FDX Client Config
CLIENTS = {
    "clientD": {"user": os.getenv("SFTP_CLIENTD_USER"), "pass": os.getenv("SFTP_CLIENTD_PASS"), "id": 11816},
    "clientE": {"user": os.getenv("SFTP_CLIENTE_USER"), "pass": os.getenv("SFTP_CLIENTE_PASS"), "id": 11817}
}
...
def get_next_controlno_from_sql():
    conn = pyodbc.connect(
        f"DRIVER={os.getenv('SQL_DRIVER')};SERVER={os.getenv('SQL_SERVER')};DATABASE={os.getenv('SQL_DATABASE')};UID={os.getenv('SQL_USERNAME')};PWD={os.getenv('SQL_PASSWORD')}"
    )
    cursor = conn.cursor()
...
def main():
    ...
    print("\n✅ All FedEx client files processed.")
